    ...
    try {
      ...
      material.onBeforeCompile = (shader) => {
        ...
        const noise = `
          ...
          float pnoise(vec3 p) {
            vec3 pi0 = mod(floor(p), 10.0);
            vec3 pi1 = mod(pi0 + 1.0, 10.0);
          ...
          float displace(vec3 point) {
            float flow = pnoise(point / 5.0 + mod(uTime, 10.0)) * 0.12;
            float surfaceNoise = pnoise(point / 0.8 + mod(uSurfaceTime, 10.0));
            float waves = (point.x * sin((point.y + surfaceNoise) * 3.1415926 * 1.35) + point.z * cos((point.y + surfaceNoise) * 3.1415926 * 1.35)) * 0.022;
            return flow + waves;
          }
        ...
        shader.vertexShader = shader.vertexShader.replace('#include <beginnormal_vertex>', `
          ...
          vec3 displacedNormal = normalize(cross(displacedNeighbour1 - displacedPosition, displacedNeighbour2 - displacedPosition));
          vec3 transformedNormal = objectNormal = displacedNormal;
        `);
      ...
      const key = new THREE.DirectionalLight(0xcaffdf, 3.8);
      key.position.set(-2, 3, 4);
      scene.add(key);
      const emerald = new THREE.PointLight(0x14ff71, 7, 8);
      emerald.position.set(2, -1, 2);
      scene.add(emerald);
      const lime = new THREE.PointLight(0x9cff55, 4, 7);
      lime.position.set(-2, 1, 1);
      scene.add(lime);
      ...
      function resize() {
        ...
        mesh.scale.setScalar(Math.min(width, height) * 0.00265);
      ...
      function draw(timestamp) {
        ...
        uniforms.uTime.value = timestamp * 0.00042;
        uniforms.uSurfaceTime.value = timestamp * 0.00092;
        uniforms.uPointer.value.set(easedX, easedY);
        mesh.rotation.y = timestamp * 0.00012 + easedX * 0.18;
        mesh.rotation.x = easedY * 0.12;
        renderer.render(scene, camera);
      }
      ...
      console.warn('Three.js liquid sphere unavailable; using existing canvas fallback.', error);
      canvas.dispatchEvent(new CustomEvent('liquid-sphere-fallback'));
    }
  }
}());
